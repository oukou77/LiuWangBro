...
extern int MagicNumber =402100;
extern int StopLoss   =800;     // SL for an opened order
extern int TakeProfit =300;     // TP for an opened order
extern bool Trailing =false;  // turn trailing on or off
extern int Trail_Stop=150;                        // Trailing distance
extern double Lots    =0.03;    // Strictly set amount of lots
extern int Slippage   =1;      // Price Slippage
   ...
   if(OrderSelect(0,SELECT_BY_POS,MODE_TRADES))
     {
      cmd=OrderType();
      //---- open order is sell
      if(cmd==OP_SELL && OrderMagicNumber()==MagicNumber)
        {
         while(true)
           {
    ...
    if(OrderSelect(0,SELECT_BY_POS,MODE_TRADES))
     {
      cmd=OrderType();
      //---- open order is buy
      if(cmd==OP_BUY && OrderMagicNumber()==MagicNumber)
        {
         while(true)
           {
            if(cmd==OP_BUY) price=Bid;
